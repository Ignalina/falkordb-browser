...
export default function Page() {
    ...
    const fetchCount = useCallback(async () => {
        ...
        const nodes = await (await securedFetch(`api/graph/${prepareArg(schemaName)}/?query=${q1}&role=${session?.user.role}`, {
            method: "GET"
        }, toast)).json()

        const edges = await (await securedFetch(`api/graph/${prepareArg(schemaName)}/?query=${q2}&role=${session?.user.role}`, {
            method: "GET"
        }, toast)).json()
    ...
    useEffect(() => {
        ...
        const run = async () => {
            const result = await securedFetch(`/api/graph/${prepareArg(schemaName)}/?query=${defaultQuery()}&role=${session?.user.role}`, {
                method: "GET"
            }, toast)
            if (!result.ok) return
